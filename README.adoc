...
and query implementations so that you easily save, find, delete and query single entities and collections of them.
...
annotation and overrides `getHosts()` and `nameSpace()` methods with the requested connection details.
...
the actual implementation, it was determined automatically by the method names behind the scenes.
...
public class PersonService {
    ...
    public void example() {
        ...
        // Get all persons that their first name starts with "Jo"
        List<Person> firstNameResults = personRepository.findByFirstnameLike("Jo*");
        // Get all persons that their last name is equal to "Smith"
...
`AerospikeOperations` is the central support interface for Aerospike database
operations which is implemented by `AerospikeTemplate` class.

It's a lower-level alternative to the AerospikeRepository, it supports a wider variety of operations,
provides more flexibility but the tradeoff is that it requires a bit more code writing and less out-of-the-box
functionality.

AerospikeOperations supported features are:

* Basic POJO mapping support to and from Bins
* Convenience CRUD (Create, Read, Update and Delete) methods to interact with Aerospike
* Rich Query API
...
* You can review questions or open a new question on
https://stackoverflow.com/questions/tagged/spring-data-aerospike[Spring Data Aerospike on StackOverFlow], and we will
try to assist as soon as possible
...
issue] for bugs and new feature requests and comment and vote on the ones that
...
to contribute code this way, please reference a GitHub issue as well
