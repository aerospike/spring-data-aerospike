...
of the `AbstractAerospikeDataConfiguration` class or define `spring-data-aerospike.hosts` and
`spring-data-aerospike.namespace` in `application.properties` file.

NOTE: You can further customize your configuration by changing xref:#configuration[`settings`] in application.properties.
