...
NOTE: "Entity" or "document" are the terms often used in Spring Data Aerospike interchangeably to describe a domain object - a Java object that identifies Aerospike DB record.
...
----
<1> Saves the given document of type `T`.
<2> Returns the DB record identified by the given ID, the record is mapped to a document of type `T`.
<3> Returns all DB records from the associated set mapped to documents of type `T`.
<4> Returns the number of all DB records in the associated set.
<5> Deletes a particular DB record using a document to identify it.
<6> Indicates whether a DB record with the given ID exists.
====
