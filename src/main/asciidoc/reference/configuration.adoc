...
This property is optional and only required when connecting to Aerospike deployments that enforce access control.
If the cluster is running without authentication enabled, this property can be omitted.

NOTE: Another way of defining username is overriding the `gatClientPolicy()` method.
It has precedence over `user` parameter from application.properties.
...
It has precedence over `password` parameter from application.properties.
