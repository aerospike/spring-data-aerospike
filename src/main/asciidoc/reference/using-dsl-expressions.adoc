...
You can declare repository queries in Spring Data Aerospike by annotating query methods with `@Query`.
...
[source,java]
----
public interface PersonDeclaredQueryRepository<P extends Person>
...
1. Selects one index to build the secondary index `Filter` (always one per query) based on index cardinality
(preferring higher `binValuesRatio`)
