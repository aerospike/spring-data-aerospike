...
If no corresponding secondary index is found, each query uses a fallback to using scan only.

For details see "Uses secondary index" column in the tables referring to queries examples below.
