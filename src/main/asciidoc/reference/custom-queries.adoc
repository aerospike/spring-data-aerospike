...
more flexible or unsupported in a standard repository. Custom queries is such a special mechanism
in Spring Data Aerospike. Currently only finding (reading) queries are supported.
...
----
        ...
        // Create secondary index filter for age >= 30
        ...
        // Verify results
        assertThat(result).containsExactlyInAnyOrderElementsOf(filteredPersons);
----
