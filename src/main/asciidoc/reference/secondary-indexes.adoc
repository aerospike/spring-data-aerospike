...
Setting secondary index via AerospikeTemplate can be helpful , for example, in cases when an index creation does not change a lot.

Here is an example of a numeric secondary index for `rating` field in `MovieDocument` entity:
...
Here is an example for the `Person` object:
...
In this case `findByLastNameContaining` query this not be satisfied by the created secondary index.
...
Hence, it will scan the repository (a costly operation which could be avoided by using an index).