...
Runnable framework-owned reference examples live in
link:https://github.com/aerospike/spring-data-aerospike/tree/main/examples[`examples/`]. They compile against the
current framework sources through the opt-in `examples` Maven profile and can be run with commands such as
`./examples/run_examples blocking-crud --hosts localhost:3000 --namespace test` (requires Maven version >= 3.9.0).
...
For copyable source that demonstrates the same repository flow, see
link:https://github.com/aerospike/spring-data-aerospike/blob/main/examples/src/main/java/org/springframework/data/aerospike/examples/blocking/crud/BlockingRepositoryCrudExample.java[`BlockingRepositoryCrudExample`].
Projection-specific repository source is available in
link:https://github.com/aerospike/spring-data-aerospike/blob/main/examples/src/main/java/org/springframework/data/aerospike/examples/blocking/projection/ProjectionExample.java[`ProjectionExample`].
