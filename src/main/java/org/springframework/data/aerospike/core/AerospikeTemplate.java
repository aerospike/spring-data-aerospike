...
public class AerospikeTemplate extends BaseAerospikeTemplate implements AerospikeOperations,
    ...
    public <T> boolean delete(Object id, Class<T> entityClass) {
        ...
        try {
            ...
            WritePolicy policy = (WritePolicy) enrichPolicyWithTransaction(client, ignoreGenerationPolicy());
    ...
    private boolean doDeleteWithVersionAndHandleCasError(AerospikeWriteData data) {
        try {
            WritePolicy wPolicy = expectGenerationPolicy(data, RecordExistsAction.UPDATE_ONLY);
            wPolicy = (WritePolicy) enrichPolicyWithTransaction(client, wPolicy);
            return client.delete(wPolicy, data.getKey());
        } catch (AerospikeException e) {
    ...
    private boolean doDeleteIgnoreVersionAndTranslateError(AerospikeWriteData data) {
        try {
            WritePolicy wPolicy = ignoreGenerationPolicy(data, RecordExistsAction.UPDATE_ONLY);
            wPolicy = (WritePolicy) enrichPolicyWithTransaction(client, wPolicy);
            return client.delete(wPolicy, data.getKey());
        } catch (AerospikeException e) {
            throw translateError(e);
    ...
    public boolean deleteById(Object id, String setName) {
        ...
        try {
            ...
            WritePolicy wPolicy = (WritePolicy) enrichPolicyWithTransaction(client, ignoreGenerationPolicy());

            return client.delete(wPolicy, key);
        } catch (AerospikeException e) {
            throw translateError(e);
        }
    }
    ...
    private void deleteAndHandleErrors(IAerospikeClient client, Key[] keys) {
        ...
        try {
            // requires server ver. >= 6.0.0
            BatchPolicy policy = (BatchPolicy) enrichPolicyWithTransaction(client, client.getBatchPolicyDefault());
            results = client.delete(policy, null, keys);
        } catch (AerospikeException e) {
            throw translateError(e);
        }
    ...
    public <T> T append(T document, String setName, Map<String, String> values) {
        ...
        try {
            ...
            WritePolicy wPolicy = (WritePolicy) enrichPolicyWithTransaction(client, client.getWritePolicyDefault());
            Record aeroRecord = client.operate(wPolicy, data.getKey(), ops);
    ...
    public <T> T append(T document, String setName, String binName, String value) {
        ...
        try {
            AerospikeWriteData data = writeData(document, setName);
            WritePolicy wPolicy = (WritePolicy) enrichPolicyWithTransaction(client, client.getWritePolicyDefault());
            Record aeroRecord = client.operate(wPolicy, data.getKey(),
    ...
    public <T, S> List<?> findByIdsUsingQuery(Collection<?> ids, Class<T> entityClass, Class<S> targetClass,
        ...
        try {
            ...
            BatchPolicy policy = (BatchPolicy) enrichPolicyWithTransaction(client, getBatchPolicyFilterExp(query));
            Class<?> target;
            Record[] aeroRecords;
            if (targetClass != null && targetClass != entityClass) {
                String[] binNames = getBinNamesFromTargetClass(targetClass, mappingContext);
                aeroRecords = client.get(policy, keys, binNames);
                target = targetClass;
            } else {
                aeroRecords = client.get(policy, keys);
                target = entityClass;
            }
    ...
    public boolean exists(Object id, String setName) {
        ...
        try {
            Key key = getKey(id, setName);
            WritePolicy wPolicy = (WritePolicy) enrichPolicyWithTransaction(client, client.getWritePolicyDefault());
            Record aeroRecord = client.operate(wPolicy, key, Operation.getHeader());
    ...
    private Record doPersistAndHandleError(AerospikeWriteData data, WritePolicy policy, Operation[] operations) {
        try {
            WritePolicy wPolicy = (WritePolicy) enrichPolicyWithTransaction(client, policy);
    ...
    private Record putAndGetHeader(AerospikeWriteData data, WritePolicy policy, boolean firstlyDeleteBins) {
        ...
        return client.operate(wPolicy, key, operations);
    }
