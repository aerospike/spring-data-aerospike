...
// Demonstrates reactive custom queries in a scan-enabled no-index context.
public class ReactiveCustomQueryNoIndexExample {

    private final ReactiveCustomQueryRepository repository;

    public ReactiveCustomQueryNoIndexExample(ReactiveCustomQueryRepository repository) {
        this.repository = repository;
    }

    public void run() {
        MovieExamples.saveMovies("reactive-custom-query-no-index", repository);
        ...
        requireTitles(repository.findUsingQuery(new Query(Qualifier.and(genre(SCIENCE_FICTION), releaseYear(1979))))
                .collectList()
                .block(),
            "No-index reactive custom query conjunction should scan", "Alien");
        ...
        requireTitles(repository.findUsingQuery(new Query(Qualifier.or(genre(CRIME), releaseYear(1979))))
                .collectList()
                .block(),
            "No-index reactive custom query disjunction should scan", "Alien", "Collateral", "Heat");
        ...
        requireTitles(repository.findUsingQuery(new Query(Qualifier.or(genre(CRIME), releaseYear(1979),
                ...
                .collectList()
                .block(),
            "No-index reactive custom query disjunction should scan",
        ...
        requireTitles(repository.findUsingQuery(new Query(Qualifier.and(genre(SCIENCE_FICTION),
                ...
                .collectList()
                .block(),
            "No-index reactive mixed custom query conjunction should scan", "Alien", "Aliens");

        System.out.println("Ran reactive combined custom queries in a scan-enabled no-index context");
    }
