...
public class ReactiveLogicalDerivedIndexedANDExample {

    private final ReactiveLogicalDerivedMovieRepository repository;

    public ReactiveLogicalDerivedIndexedANDExample(ReactiveLogicalDerivedMovieRepository repository) {
        this.repository = repository;
    }

    public void run() {
        LogicalMovieExamples.saveMovies("reactive-derived-indexed-and", repository);
        ...
        requireTitles(repository.findByGenreAndReleaseYear(of(SCIENCE_FICTION), of(1979))
                .collectList()
                .block(),
            "One reactive derived AND query should use the lGenre index", "Alien");
        ...
        requireTitles(repository.findByGenreAndReleaseYearAndTitle(
                ...
                .collectList()
                .block(),
            "Multiple reactive derived AND query should use the lTitle index", "Alien");

        System.out.println("Ran reactive derived AND queries backed by the lGenre and lTitle indexes");
    }
}
