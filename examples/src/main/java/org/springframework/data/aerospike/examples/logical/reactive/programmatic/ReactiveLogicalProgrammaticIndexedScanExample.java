...
public class ReactiveLogicalProgrammaticIndexedScanExample {

    private final ReactiveLogicalProgrammaticMovieRepository repository;

    public ReactiveLogicalProgrammaticIndexedScanExample(ReactiveLogicalProgrammaticMovieRepository repository) {
        this.repository = repository;
    }

    public void run() {
        LogicalMovieExamples.saveMovies("reactive-programmatic-indexed-scan", repository);
        ...
        requireTitles(repository.findUsingQuery(oneOr).collectList().block(),
            "One reactive programmatic OR query should scan even with an lGenre index",
        ...
        requireTitles(repository.findUsingQuery(multipleOr).collectList().block(),
            "Multiple reactive programmatic OR query should scan even with an lGenre index",
        ...
        System.out.println("Ran reactive programmatic OR queries in a scan-enabled indexed context");
    }
