...
public class BlockingLogicalProgrammaticNoIndexExample {

    private final BlockingLogicalProgrammaticMovieRepository repository;

    public BlockingLogicalProgrammaticNoIndexExample(BlockingLogicalProgrammaticMovieRepository repository) {
        this.repository = repository;
    }

    public void run() {
        LogicalMovieExamples.saveMovies("blocking-programmatic-no-index", repository);
        ...
        requireTitles(repository.findUsingQuery(new Query(Qualifier.and(genre(SCIENCE_FICTION), releaseYear(1979)))),
            "No-index programmatic one AND query should scan", "Alien");
        ...
        requireTitles(repository.findUsingQuery(new Query(Qualifier.or(genre(CRIME), releaseYear(1979)))),
            "No-index programmatic one OR query should scan", "Alien", "Collateral", "Heat");

        // Multiple AND remains expression-only because lGenre was deliberately not indexed.
        requireTitles(repository.findUsingQuery(new Query(Qualifier.and(genre(SCIENCE_FICTION), releaseYear(1979),
                title("Alien")))),
            "No-index programmatic multiple AND query should scan", "Alien");

        // Multiple OR remains expression-only and scan-backed.
        requireTitles(repository.findUsingQuery(new Query(Qualifier.or(genre(CRIME), releaseYear(1979),
                title("Network")))),
            "No-index programmatic multiple OR query should scan", "Alien", "Collateral", "Heat", "Network");
        ...
        requireTitles(repository.findUsingQuery(new Query(Qualifier.and(genre(SCIENCE_FICTION),
            ...
            "No-index programmatic mixed query should scan", "Alien", "Aliens");

        System.out.println("Ran blocking programmatic logical queries in a scan-enabled no-index context");
    }
